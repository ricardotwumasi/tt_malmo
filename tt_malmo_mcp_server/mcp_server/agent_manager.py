...
from llm_adapters import ClaudeAdapter, GeminiAdapter
...
class AgentManager:
    ...
    async def create_agent(self, name: str, llm_type: str = "claude",
                          role: int = 0, traits: Optional[List[str]] = None) -> str:
        """
        Create a new agent with PIANO architecture.

        Args:
            name: Agent name
            llm_type: LLM type ("claude" or "gemini")
        ...
        if llm_type == "claude":
            ...
            llm_adapter = GeminiAdapter(
                ...
                # Uses gemini-1.5-flash-latest by default (free tier)
                max_tokens=512
            )
        else:
